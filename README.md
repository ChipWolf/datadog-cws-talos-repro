# Datadog CWS Runtime Fails on Talos Linux: kprobe Symbol Mismatch

Minimal reproduction and root cause analysis for Datadog Agent Cloud Workload
Security (CWS) runtime failing on Talos Linux due to a GCC optimization renaming
a kernel symbol that Datadog's eBPF kprobe attaches to by exact name.
...
```
GCC IPA-SRA optimization during Talos kernel compilation
    |
    v
Kernel symbol renamed: attach_recursive_mnt --> attach_recursive_mnt.isra.0
    |
    v
Datadog system-probe tries kprobe on exact name "attach_recursive_mnt"
    |
    v
Linux kprobe subsystem: exact name lookup fails (symbol doesn't exist without suffix)
    |
    v
event_monitor module fails to register (AllOf selector requires ALL probes to attach)
    |
    v
runtime-security.sock is never created
    |
    v
security-agent retries connecting to missing socket forever
```
